"""Legacy joint VQ/GRU trajectory-only USC-HAD continual-GCD stream.

This implementation is retained for historical reproduction and is no longer
the public HHR entry point. Activity labels remain available to the protocol audit and final evaluator,
...
from experiments.motion_primitive.legacy_profiles import PROFILE_JOINT, normalize_profile  # noqa: E402
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    identity: dict[str, Any] = {
        ...
        "source_sha256": {
            ...
            "experiments/motion_primitive/legacy_profiles.py": _sha256_file(
                Path(__file__).with_name("legacy_profiles.py")
            ),
            "data/uschad_har.py": _sha256_file(
                PROJECT_ROOT / "data" / "uschad_har.py"
            ),
            "data/uschad.py": _sha256_file(
                PROJECT_ROOT / "data" / "uschad.py"
            ),
            "models/motion_primitive_cgcd.py": _sha256_file(
                PROJECT_ROOT / "models" / "motion_primitive_cgcd.py"
            ),
            "models/motion_primitives.py": _sha256_file(
                PROJECT_ROOT / "models" / "motion_primitives.py"
            ),
            "models/resnet1d.py": _sha256_file(
                PROJECT_ROOT / "models" / "resnet1d.py"
            ),
            "models/batch_utils.py": _sha256_file(
