...
class OneStageCVRunnerTests(unittest.TestCase):
    ...
    def test_dry_run_builds_exact_seven_fold_two_profile_grid(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue(all("--codebook-update ema" in item for item in joined))
            ...
            self.assertEqual(
                result["identity"]["schema"], "one_stage_motion_trajectory_cv_v4"
            )
            self.assertTrue(
    ...
    def test_member_artifact_manifest_is_exhaustively_verified(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            identity_payload = {
                "schema": "one_stage_motion_trajectory_run_v4",
                "profile": "J0-U",
                "fold": 1,
                "seed": 50,
