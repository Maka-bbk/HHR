...
def _inputs(*, with_trajectory: bool = False) -> OneStageLossInputs:
    ...
    assignment_logits = torch.randn(
        batch, length, code_count, requires_grad=True
    )
    posterior = torch.softmax(assignment_logits, dim=-1)
    hard = F.one_hot(
        posterior.argmax(dim=-1), num_classes=code_count
    ).to(posterior.dtype)
    trajectory_assignments = posterior + (hard - posterior).detach()
    trajectory_assignments = trajectory_assignments.masked_fill(
        ~valid_window.unsqueeze(-1), 0.0
    )
    return OneStageLossInputs(
        ...
        normalized_codebook=F.normalize(
            torch.randn(code_count, feature_dim, requires_grad=True), dim=-1
        ),
...
class J0UnsupervisedLossTests(unittest.TestCase):
    def test_j0_u_without_labels_trains_local_codebook_and_trajectory_paths(self):
        ...
        self.assertGreater(float(inputs.local_features.grad.abs().sum()), 0.0)
...
class CodebookAndStructureLossTests(unittest.TestCase):
    def test_straight_through_utilization_sees_the_actual_hard_collapse(self):
        valid = torch.ones(2, 6, dtype=torch.bool)
        logits = torch.zeros(2, 6, 4, requires_grad=True)
        with torch.no_grad():
            logits[..., 0] = 0.01
        posterior = torch.softmax(logits, dim=-1)
        hard = F.one_hot(posterior.argmax(dim=-1), num_classes=4).to(
            posterior.dtype
        )
        assignments = posterior + (hard - posterior).detach()
        loss, metrics = straight_through_codebook_utilization_floor_loss(
            assignments,
            valid,
            normalized_entropy_floor=0.50,
        )
        self.assertGreater(float(loss.detach()), 0.0)
        self.assertAlmostEqual(metrics["effective_code_count"], 1.0, places=6)
        loss.backward()
        self.assertGreater(float(logits.grad.abs().sum()), 0.0)

    def test_near_uniform_single_argmax_is_not_mistaken_for_healthy_usage(self):
        valid = torch.ones(2, 6, dtype=torch.bool)
        logits = torch.zeros(2, 6, 4, requires_grad=True)
        with torch.no_grad():
            logits[..., 0] = 0.01
        ...
        confidence, conditional_entropy = assignment_confidence_loss(
            logits,
            valid,
            maximum_normalized_entropy=0.50,
        )
        self.assertEqual(float(utilization.detach()), 0.0)
        self.assertGreater(utilization_metrics["effective_code_count"], 3.99)
        self.assertTrue(bool(torch.all(logits.argmax(dim=-1) == 0)))
        self.assertGreater(float(confidence.detach()), 0.0)
        self.assertGreater(conditional_entropy, 0.99)

    def test_raw_feature_noncollapse_penalizes_constant_representation(self):
        valid = torch.ones(4, 4, dtype=torch.bool)
        collapsed = torch.ones(4, 4, 8, requires_grad=True)
        loss, metrics = trial_balanced_feature_noncollapse_loss(
            collapsed,
            valid,
            target_std=1.0,
            variance_weight=1.0,
        ...
        self.assertGreater(float(loss.detach()), 0.9)
        self.assertLess(metrics["feature_noncollapse_mean_std"], 0.02)
        loss.backward()
        self.assertIsNotNone(collapsed.grad)

    def test_codebook_diversity_rejects_duplicate_centres(self):
        duplicate = F.normalize(torch.ones(4, 8), dim=-1).requires_grad_()
        separated = torch.eye(4, 8, requires_grad=True)
        duplicate_loss, duplicate_metrics = codebook_diversity_loss(
            duplicate, maximum_cosine=0.25
        )
        separated_loss, _ = codebook_diversity_loss(
            separated, maximum_cosine=0.25
        )
        self.assertGreater(float(duplicate_loss.detach()), 0.0)
        self.assertGreater(duplicate_metrics["codebook_pair_cosine_mean"], 0.99)
        self.assertEqual(float(separated_loss.detach()), 0.0)
...
class J0TrajectoryLossTests(unittest.TestCase):
    ...
    def test_all_unlabelled_batch_returns_graph_connected_zero_ce(self):
        logits = torch.randn(3, 3, requires_grad=True)
        labels = torch.full((3,), -1, dtype=torch.long)
        selected = torch.zeros(3, dtype=torch.bool)
        loss, labelled_count = trajectory_old_class_cross_entropy(
            logits,
            labels,
            selected,
            old_class_count=3,
        )
        self.assertEqual(labelled_count, 0)
        self.assertEqual(float(loss.detach()), 0.0)
        loss.backward()
