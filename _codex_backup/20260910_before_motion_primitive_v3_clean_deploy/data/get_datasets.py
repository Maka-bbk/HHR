...
from data.uschad import get_uschad_datasets
...
from data.uschad import subsample_classes as subsample_dataset_uschad
...
sub_sample_class_funcs = {
    ...
    'uschad': subsample_dataset_uschad,
}
...
def get_datasets(dataset_name, train_transform, test_transform, args):
    ...
    use_trial_counts = (
        dataset_name == 'uschad'
        and getattr(args, 'uschad_sample_unit', 'window') == 'trial'
    )
    dataset_split_config_dict = {
        ...
        'online_novel_unseen_num': (
            ...
            if use_trial_counts else args.online_novel_unseen_num
        ),
        'online_old_seen_num': (
            args.online_old_seen_trials
            if use_trial_counts else args.online_old_seen_num
        ),
        'online_novel_seen_num': (
            args.online_novel_seen_trials
            if use_trial_counts else args.online_novel_seen_num
        ),
        'sample_unit': 'trial' if use_trial_counts else 'window',
    }
    ...
    offline_session_val_dataset = datasets.get('offline_val_dataset')
    ####################################################################################################################
    ...
    return  offline_session_train_dataset, offline_session_val_dataset, offline_session_test_dataset,\
...
def get_class_splits(args):
    ...
    if args.dataset_name == 'cifar10':
    ...
    elif args.dataset_name == 'uschad':

        # USC-HAD labels are converted to 0-based labels in preprocess_uschad.py.
        # Original USC-HAD activity ids:
        #   1-6  -> old classes
        #   7-12 -> new classes
        # Code labels:
        #   0-5  -> old classes
        ...
        args.image_size = getattr(args, 'uschad_window_size', 256)

        args.train_classes = range(6)
        args.unlabeled_classes = range(6, 12)

        if args.num_old_classes > 0:
            full_classes = list(range(12))
            args.train_classes = full_classes[:args.num_old_classes]
            args.unlabeled_classes = full_classes[args.num_old_classes:]
