...
CV_SCHEMA_VERSION = "one_stage_motion_trajectory_cv_v5"
PROFILES = ("J0-U", "J0-T")
...
def _command(args: argparse.Namespace, profile: str, fold: int, seed: int, output: Path) -> list[str]:
    command = [
        ...
        "--revive-unused-codes"
        if args.revive_unused_codes
        else "--no-revive-unused-codes",
...
def _load_rows(
    ...
    *,
    ...
    expected_codebook_init: str | None = None,
    ...
    for profile in profiles:
        for fold in folds:
            for seed in seeds:
                ...
                codebook_init = str(summary.get("codebook_init", ""))
                codebook_update = str(summary.get("codebook_update", ""))
                revive_unused_codes = summary.get("revive_unused_codes")
                ...
                if (
                    ...
                    or manifest_training_parameters.get("codebook_init")
                    != codebook_init
                    or manifest_training_parameters.get("codebook_update")
                    != codebook_update
                    or manifest_training_parameters.get("revive_unused_codes")
                    != revive_unused_codes
                ):
                    raise RuntimeError(
                ...
                if (
                    expected_codebook_init is not None
                    and codebook_init != expected_codebook_init
                ):
                    raise RuntimeError(
                        f"Unexpected codebook initialization in {summary_path}: "
                        f"{codebook_init!r}."
                    )
                if (
                ...
                if primary_arm != (
                    ...
                    and codebook_init == "learnable"
                    and codebook_update == "gradient"
                    and revive_unused_codes is False
                ...
                row = {
                    ...
                    "revive_unused_codes": revive_unused_codes,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Loss-weight ramp only; not ResNet1D pretraining.",
    )
    ...
    parser.add_argument(
        "--codebook-init",
        choices=("learnable", "kmeans++", "random"),
        default="learnable",
    )
    ...
    parser.add_argument(
        "--codebook-update", choices=CODEBOOK_UPDATE_MODES, default="gradient"
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for name in ("num_workers", "loss_ramp_epochs", "early_stop_patience", "aggregate_seed"):
    ...
    rows = _load_rows(
        ...
        expected_codebook_init=str(args.codebook_init),
