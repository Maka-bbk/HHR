...
def _trial_balanced_masked_mean(
    values: torch.Tensor, valid_mask: torch.Tensor
) -> torch.Tensor:
    """Average positions within each trial, then average trials equally."""

    if values.ndim != 2 or tuple(values.shape) != tuple(valid_mask.shape):
        raise ValueError("values and valid_mask must share shape [B,L].")
    weights = valid_mask.to(values.dtype)
    counts = weights.sum(dim=1)
    selected = counts > 0
    if not torch.any(selected):
        return _graph_zero(values)
    per_trial = (values * weights).sum(dim=1) / counts.clamp_min(1.0)
    return per_trial[selected].mean()
...
class OneStageLossConfig:
    ...
    feature_noncollapse_weight: float = 2.00
    utilization_floor_weight: float = 1.00
    # Marginal utilization and conditional confidence are the two terms of a
    # clustering mutual-information objective.  Equal default scale avoids the
    # previous 5:1 winner-take-all pressure while still allowing both terms to
    # be ablated independently.
    assignment_confidence_weight: float = 1.00
    codebook_diversity_weight: float = 0.50
    ...
    noncollapse_target_std: float = 1.0
    noncollapse_variance_weight: float = 1.0
    noncollapse_covariance_weight: float = 1.0
    noncollapse_windows_per_trial: int = 4
    ...
    def validated(self) -> "OneStageLossConfig":
        ...
        if not 0.0 <= float(self.maximum_assignment_entropy) <= 1.0:
            raise ValueError("maximum_assignment_entropy must lie in [0,1].")
        if not -1.0 <= float(self.maximum_codebook_cosine) <= 1.0:
            raise ValueError("maximum_codebook_cosine must lie in [-1,1].")
        if not math.isfinite(float(self.noncollapse_target_std)) or float(
            self.noncollapse_target_std
        ) <= 0.0:
            raise ValueError("noncollapse_target_std must be positive and finite.")
        _finite_nonnegative(
            "noncollapse_variance_weight", self.noncollapse_variance_weight
        )
        _finite_nonnegative(
            "noncollapse_covariance_weight", self.noncollapse_covariance_weight
        )
        if int(self.noncollapse_windows_per_trial) < 1:
            raise ValueError("noncollapse_windows_per_trial must be positive.")
        ...
        result.update(
            {
                ...
                "utilization_regularizer": (
                    "trial-balanced straight-through hard-assignment marginal "
                    "entropy floor plus conditional soft-assignment confidence ceiling"
                ),
                "objective_scopes": {
                    ...
                    "feature_noncollapse": (
                        "trial-balanced post-LayerNorm/pre-L2 local features; VICReg-style "
                        "variance/covariance"
                    ),
                    "utilization_floor": (
                        "trial-balanced marginal entropy of deterministic straight-through "
                        "hard VQ assignments (hard forward, soft backward)"
                    ),
                    "assignment_confidence": (
                        "mean conditional entropy ceiling; closes uniform-soft/"
                        "single-argmax loophole"
                    ),
                    "codebook_diversity": "pairwise normalized code-vector separation",
...
class OneStageLossInputs:
    ...
    trajectory_assignments: torch.Tensor
...
def vq_losses(
    ...
    if not torch.any(valid):
        ...
        return zero, zero
    commitment_values = (
        encoded_states - quantized_states.detach()
    ).square().mean(dim=-1)
    codebook_values = (
        quantized_states - encoded_states.detach()
    ).square().mean(dim=-1)
    commitment = _trial_balanced_masked_mean(commitment_values, valid)
    codebook = _trial_balanced_masked_mean(codebook_values, valid)
    return commitment, codebook


def trial_balanced_feature_noncollapse_loss(
    local_features: torch.Tensor,
    valid_window_mask: torch.Tensor,
    *,
    target_std: float,
    ...
    """VICReg-style anti-collapse on a trial-balanced window sample.

    The loss is global across trials rather than within each trial, so a
    legitimately simple or nearly static activity is not forced to oscillate.
    """

    _floating("local_features", local_features, 3)
    valid = _boolean_mask(
        "valid_window_mask", valid_window_mask, tuple(local_features.shape[:2])
    )
    if valid.device != local_features.device:
        raise ValueError("local_features and valid_window_mask must share a device.")
    ...
    if not math.isfinite(float(target_std)) or float(target_std) <= 0.0:
        raise ValueError("target_std must be positive and finite.")
    variance_weight = _finite_nonnegative("variance_weight", variance_weight)
    covariance_weight = _finite_nonnegative("covariance_weight", covariance_weight)
    selected: list[torch.Tensor] = []
    for row in range(local_features.shape[0]):
        length = int(valid[row].sum().item())
        if length < 1:
            continue
        count = min(int(windows_per_trial), length)
        positions = torch.linspace(
            0, length - 1, steps=count, device=valid.device
        ).round().long()
        selected.append(local_features[row, positions])
    if sum(len(item) for item in selected) < 2:
        zero = _graph_zero(local_features)
        return zero, {
            "feature_noncollapse_variance": 0.0,
            "feature_noncollapse_covariance": 0.0,
            "feature_noncollapse_mean_std": 0.0,
            "feature_noncollapse_sample_count": float(sum(len(item) for item in selected)),
        }
    values = torch.cat(selected, dim=0)
    centred = values - values.mean(dim=0, keepdim=True)
    variance = centred.square().sum(dim=0) / max(1, len(values) - 1)
    standard_deviation = torch.sqrt(variance + 1.0e-4)
    variance_loss = F.relu(
        standard_deviation.new_tensor(float(target_std)) - standard_deviation
    ).mean()
    covariance = centred.transpose(0, 1) @ centred / max(1, len(values) - 1)
    off_diagonal = covariance - torch.diag_embed(torch.diagonal(covariance))
    covariance_loss = off_diagonal.square().sum() / max(1, values.shape[1])
    loss = variance_weight * variance_loss + covariance_weight * covariance_loss
    return loss, {
        "feature_noncollapse_variance": float(variance_loss.detach().cpu()),
        "feature_noncollapse_covariance": float(covariance_loss.detach().cpu()),
        "feature_noncollapse_mean_std": float(standard_deviation.mean().detach().cpu()),
        "feature_noncollapse_sample_count": float(len(values)),
    }


def assignment_confidence_loss(
    assignment_logits: torch.Tensor,
    valid_window_mask: torch.Tensor,
    *,
    ...
    if code_count <= 1 or not torch.any(valid):
        return _graph_zero(assignment_logits), 0.0
    probabilities = F.softmax(assignment_logits / float(temperature), dim=-1)
    conditional = -(
        probabilities
        * probabilities.clamp_min(torch.finfo(probabilities.dtype).tiny).log()
    ).sum(dim=-1) / math.log(code_count)
    mean_entropy = _trial_balanced_masked_mean(conditional, valid)
    loss = F.relu(mean_entropy - mean_entropy.new_tensor(ceiling)).square()
    return loss, float(mean_entropy.detach().cpu())


def codebook_diversity_loss(
    normalized_codebook: torch.Tensor, *, maximum_cosine: float
) -> tuple[torch.Tensor, dict[str, float]]:
    """Keep distinct code vectors from converging to the same direction."""

    _floating("normalized_codebook", normalized_codebook, 2)
    if normalized_codebook.shape[0] <= 1:
        zero = _graph_zero(normalized_codebook)
        return zero, {"codebook_pair_cosine_mean": 0.0, "codebook_pair_cosine_max": 0.0}
    maximum = float(maximum_cosine)
    if not -1.0 <= maximum <= 1.0:
        raise ValueError("maximum_cosine must lie in [-1,1].")
    vectors = F.normalize(normalized_codebook, dim=-1)
    similarity = vectors @ vectors.transpose(0, 1)
    off_diagonal = similarity[~torch.eye(
        len(vectors), dtype=torch.bool, device=vectors.device
    )]
    loss = F.relu(off_diagonal - maximum).square().mean()
    return loss, {
        "codebook_pair_cosine_mean": float(off_diagonal.mean().detach().cpu()),
        "codebook_pair_cosine_max": float(off_diagonal.max().detach().cpu()),
    }
...
def codebook_utilization_floor_loss(
    ...
    *,
    ...
    probabilities = F.softmax(assignment_logits / float(temperature), dim=-1)
    weights = valid.to(probabilities.dtype).unsqueeze(-1)
    per_trial_usage = (probabilities * weights).sum(dim=1) / weights.sum(
        dim=1
    ).clamp_min(1.0)
    nonempty = valid.any(dim=1)
    usage = per_trial_usage[nonempty].mean(dim=0)
...
def straight_through_codebook_utilization_floor_loss(
    trajectory_assignments: torch.Tensor,
    valid_window_mask: torch.Tensor,
    *,
    normalized_entropy_floor: float,
) -> tuple[torch.Tensor, dict[str, float]]:
    """Put the utilization floor on the discrete trajectory itself.

    The primary model supplies exact one-hot values in the forward pass and
    soft-posterior gradients in the backward pass.  Consequently this loss
    cannot be satisfied by a nearly uniform posterior whose argmax is the
    same code at every window.
    """

    _floating("trajectory_assignments", trajectory_assignments, 3)
    valid = _boolean_mask(
        "valid_window_mask",
        valid_window_mask,
        tuple(trajectory_assignments.shape[:2]),
    )
    if valid.device != trajectory_assignments.device:
        raise ValueError("valid_window_mask must share the assignment device.")
    floor = float(normalized_entropy_floor)
    if not 0.0 <= floor <= 1.0:
        raise ValueError("normalized_entropy_floor must lie in [0,1].")
    code_count = int(trajectory_assignments.shape[-1])
    if code_count < 1:
        raise ValueError("trajectory_assignments must contain at least one code.")
    selected = trajectory_assignments[valid]
    if torch.any(selected < 0.0):
        raise ValueError("Valid trajectory assignments must be non-negative.")
    if len(selected) and not torch.allclose(
        selected.sum(dim=-1),
        torch.ones(len(selected), device=selected.device, dtype=selected.dtype),
        atol=1.0e-5,
        rtol=1.0e-5,
    ):
        raise ValueError("Valid trajectory assignment rows must sum to one.")
    if not torch.any(valid) or code_count == 1:
        zero = _graph_zero(trajectory_assignments)
        return zero, {"normalized_codebook_entropy": 1.0, "effective_code_count": 1.0}
    weights = valid.to(trajectory_assignments.dtype).unsqueeze(-1)
    per_trial_usage = (trajectory_assignments * weights).sum(dim=1) / weights.sum(
        dim=1
    ).clamp_min(1.0)
    usage = per_trial_usage[valid.any(dim=1)].mean(dim=0)
    entropy = -(usage * usage.clamp_min(torch.finfo(usage.dtype).tiny).log()).sum()
    normalized = entropy / math.log(code_count)
    loss = F.relu(normalized.new_tensor(floor) - normalized).square()
    return loss, {
...
def state_reconstruction_loss(
    ...
    *,
    ...
    elementwise = F.huber_loss(
        ...
        state_targets.detach(),
        reduction="none",
        delta=delta,
    )
    per_window = elementwise.flatten(start_dim=2).mean(dim=-1)
    return _trial_balanced_masked_mean(per_window, valid)
...
def trajectory_old_class_cross_entropy(
    ...
    *,
    ...
    if not labelled.numel():
        # Stratified labelling guarantees labelled trials at dataset level, not
        # in every randomly shuffled mini-batch.  Returning a graph-connected
        # zero keeps the J0-T objective well defined without changing the
        # sampler distribution or leaking labels from unlabelled rows.
        return trajectory_logits.sum() * 0.0, 0
...
def compose_one_stage_loss(
    ...
    _floating("local_features", inputs.local_features, 3)
    if tuple(inputs.local_features.shape[:2]) != (batch_size, window_count):
        raise ValueError("local_features must align with encoded window positions.")
    _floating("normalized_codebook", inputs.normalized_codebook, 2)
    if inputs.normalized_codebook.shape[1] != inputs.encoded_states.shape[2]:
        raise ValueError("normalized_codebook width must equal encoded state width.")
    ...
    if tuple(inputs.trajectory_assignments.shape) != tuple(inputs.assignment_logits.shape):
        raise ValueError(
            "trajectory_assignments must align with assignment_logits [B,L,K]."
        )
    ...
    feature_noncollapse, feature_noncollapse_metrics = (
        trial_balanced_feature_noncollapse_loss(
            inputs.local_features,
            inputs.valid_window_mask,
            target_std=float(config.noncollapse_target_std),
            variance_weight=float(config.noncollapse_variance_weight),
            covariance_weight=float(config.noncollapse_covariance_weight),
            windows_per_trial=int(config.noncollapse_windows_per_trial),
        )
    )
    utilization, utilization_metrics = straight_through_codebook_utilization_floor_loss(
        inputs.trajectory_assignments,
        inputs.valid_window_mask,
        normalized_entropy_floor=float(config.utilization_entropy_floor),
    )
    _, soft_utilization_metrics = codebook_utilization_floor_loss(
        inputs.assignment_logits,
        inputs.valid_window_mask,
        normalized_entropy_floor=float(config.utilization_entropy_floor),
        temperature=float(config.utilization_temperature),
    )
    assignment_confidence, normalized_assignment_entropy = assignment_confidence_loss(
        inputs.assignment_logits,
        inputs.valid_window_mask,
        maximum_normalized_entropy=float(config.maximum_assignment_entropy),
        temperature=float(config.utilization_temperature),
    )
    codebook_diversity, codebook_diversity_metrics = codebook_diversity_loss(
        inputs.normalized_codebook,
        maximum_cosine=float(config.maximum_codebook_cosine),
    )
    ...
    components = {
        ...
        "feature_noncollapse": feature_noncollapse,
    ...
    weights = {
        ...
        "feature_noncollapse": float(config.feature_noncollapse_weight),
        "utilization_floor": float(config.utilization_floor_weight),
        "assignment_confidence": float(config.assignment_confidence_weight),
        "codebook_diversity": float(config.codebook_diversity_weight),
    ...
    metrics: dict[str, float | int | None] = {
        **boundary_metrics,
        **utilization_metrics,
        "normalized_soft_codebook_entropy": soft_utilization_metrics[
            "normalized_codebook_entropy"
        ],
        "soft_effective_code_count": soft_utilization_metrics[
            "effective_code_count"
        ],
        **feature_noncollapse_metrics,
        **codebook_diversity_metrics,
        "normalized_assignment_entropy": normalized_assignment_entropy,
...
__all__ = [
    ...
    "straight_through_codebook_utilization_floor_loss",
