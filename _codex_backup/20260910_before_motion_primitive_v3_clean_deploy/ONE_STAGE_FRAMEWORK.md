...
本文档描述当前实现，而不是尚未实现的设想。J0 的目标是用随机初始化的 ResNet1D（一维残差网络）和随机初始化的可学习归一化动作元原型、一个优化器及一条连续检查点（checkpoint，模型检查点）谱系，同时学习局部内容、边界、向量量化（vector quantization，VQ）码本、物理状态和完整试次轨迹。编码器与原型从第一个 epoch（训练轮次）联合更新，没有独立编码器预热，也不加载 HAPPY 或 A2 检查点。
...
- J0 只借鉴 HAPPY 的头部架构原则：L2 归一化局部特征与归一化无偏置原型进行相似度匹配，并让编码器和原型头联合优化。它没有继承 HAPPY 的窗口目标。原版 HAPPY 的 ViT 使用外部 DINO 预训练权重；USC-HAD 版 ResNet1D 则从头随机初始化，因此“没有独立预热”和“参数不是随机初始化”不是同一件事。
...
- J0 已把“表示学习→边界→码本→轨迹编码”缩成一次连续训练；训练中没有先训编码器再冻结做码本的第二阶段。默认也不再在未训练 ResNet1D 的输出上做 KMeans++ 初始化。训练前的原始运动学校准是确定性变换，不是预训练。
...
- 默认 `codebook-init=learnable`、`codebook-update=gradient`：ResNet1D 和动作元原型从第 1 轮共同进入 AdamW；原型每步投影回单位球面，对应 HAPPY 固定增益的 weight normalization（权重归一化）原型头。`kmeans++` 与 `ema` 仅是消融，不定义主方法。
- 训练、验证和外层测试受试者互斥。
...
默认不使用 Gumbel-Softmax（耿贝尔软最大）。训练前向和评价都采用确定性 argmax（最大值索引）one-hot（独热）动作元；训练反向通过温度退火的 soft posterior（软后验）传播直通梯度。这样轨迹编码器始终看见离散编号，同时原型和 ResNet1D 从第 1 轮联合优化。温度从 2.0 几何退火至 0.25，使反向分配由平滑逐渐变尖锐。按连续相同标记或预测边界形成的变长动作元区间用于评价、可视化和导出。原始边界锚点只覆盖完整原始帧；额外右侧 padding 尾帧对应的边界保持不确定，不进入边界监督。
...
| 6 | `feature_noncollapse` | 2.00 | 试次等权的 VICReg 式方差／协方差约束直接作用于 L2 归一化前的局部特征，防止连续表征退化为单一方向。 |
| 7 | `utilization_floor` | 1.00 | 对实际直通硬轨迹的试次等权边际熵设置 0.50 下限；达到下限后不继续强迫所有动作元等频。 |
| 8 | `assignment_confidence` | 1.00 | 当每个窗口的软后验归一化熵高于 0.50 时处罚，避免“软占用看似完整、硬 argmax 全落一元”；与利用率项采用 1:1 基础权重，避免旧 5:1 配置产生赢家通吃。 |
| 9 | `codebook_diversity` | 0.50 | 对动作元原型两两余弦相似度设置 0.25 上限，防止多个编号实际对应相同方向。 |
| 10 | `boundary_sparsity` | 0.02 | 排除 q90 锚点后，预测边界率高于 0.35 才处罚的单侧预算。 |
| 11 | `minimum_duration` | 0.05 | 软性抑制边缘处和相邻过密边界；默认最短 2 个模型帧。 |
| 12 | `state_reconstruction` | 0.50 | 从码字重建无标签物理状态描述符；目标停止梯度。 |
| 13 | `masked_token_prediction` | 1.00 | 在掩码有效轨迹位置预测停止梯度的整数 VQ 伪标记；默认掩码比例 0.15。 |
| 14 | `masked_state_reconstruction` | 0.50 | 在相同掩码位置重建停止梯度的物理状态目标。 |
| 15 | `trajectory_ce` | J0-U: 0；J0-T: 1.00 | 只对显式 `trajectory_label_mask=true` 的旧类完整试次计算交叉熵。 |

J0-U 不只是把第 15 项乘零：它从接口层禁止 `trajectory_logits`、活动标签和有标签 mask 进入损失。J0-T 则强制第 15 项权重大于零，并强制未标注行只能携带 `-100`。两种配置均不包含 HAPPY/A2 的隐藏附加项；`run_manifest.json` 中的 `loss_config` 会写出完整审计字段。

### 连续课程与验证权重

默认 `loss_ramp_epochs=10`。第 `e` 个 epoch 使用

```text
r(e) = max(0.05, min(1, e / loss_ramp_epochs))
```

`stable_next_content`、`masked_token_prediction`、`masked_state_reconstruction`、`assignment_confidence`，以及 J0-T 的 `trajectory_ce` 乘以 `r(e)`；其余项保持基础权重。同一模型和优化器不重置，所以这只是 loss ramp（损失权重渐增），不是编码器预热或第二训练阶段。码本反向温度默认从 2.0 几何退火到 0.25。

每轮验证始终使用表中的完整基础权重和硬码本，不用当轮的渐增权重。默认 `checkpoint-selection=common_unsupervised`：J0-U 与 J0-T 都先按较低的无标签验证目标选点，再以较高有效码字数打破平局；因此两臂不会因为 J0-T 使用验证活动标签选点而产生额外优势。`profile_default` 只保留为诊断消融，其中 J0-T 才按 validation trajectory macro-F1（验证轨迹宏平均 F1）选点。
...
```bash
cd /mnt/d/WorkDir/HHR && /home/bj/miniconda3/envs/hhr/bin/python experiments/motion_primitive/run_one_stage_cv.py --data-root /mnt/d/WorkDir/DataSet/USC-HAD --output-root /mnt/d/WorkDir/HHR/results/motion_primitive/one_stage_happy_style_joint_J0_U_T_7fold_seed50_v3 --profiles J0-U,J0-T --folds 1,2,3,4,5,6,7 --seeds 50 --epochs 80 --batch-size 8 --eval-batch-size 16 --num-workers 0 --device cuda --learning-rate 3e-4 --weight-decay 1e-4 --gradient-clip 5 --loss-ramp-epochs 10 --early-stop-patience 20 --frame-size 128 --frame-stride 64 --local-encoder resnet1d --codebook-size 32 --codebook-init learnable --codebook-update gradient --trajectory-input-mode primitive_only --checkpoint-selection common_unsupervised --temperature-start 2 --temperature-end 0.25 --trajectory-mask-ratio 0.15 --labelled-fraction 0.8 --anomaly-policy report --cluster-restarts 10 --order-shuffles 10 --minimum-segment-windows 2 --bootstrap-replicates 10000 --aggregate-seed 20260909 --deterministic --resume
```
