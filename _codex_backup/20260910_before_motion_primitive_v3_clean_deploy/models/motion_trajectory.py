...
LOCAL_ENCODER_TYPES = ("light_cnn", "resnet1d")
...
class MotionTrajectoryConfig:
    ...
    local_encoder_type: str = "resnet1d"
    state_hidden_dim: int = 32
    state_dim: int = 12
    codebook_size: int = 32
    codebook_temperature: float = 0.25
    # Deterministic straight-through assignments are the primary protocol.
    # Stochastic Gumbel assignments remain available only as an explicit
    # ablation because they can hide deterministic argmax collapse.
    ...
    def validated(self) -> "MotionTrajectoryConfig":
        ...
        if self.local_encoder_type not in LOCAL_ENCODER_TYPES:
            raise ValueError(
                f"local_encoder_type must be one of {LOCAL_ENCODER_TYPES}."
            )
...
class LocalTemporalEncoder(nn.Module):
    ...
    def _pooled_features(
        ...
        return pooled.reshape(batch_size, token_count, -1)

    def forward_with_raw(
        self, windows: torch.Tensor, sample_mask: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Return LayerNorm output and the pre-normalization representation.

        Collapse prevention must act on the raw representation.  Applying a
        variance objective after LayerNorm can report healthy coordinate scale
        even when every window has converged to the same direction.
        """

        raw = self._pooled_features(windows, sample_mask)
        return self.output_norm(raw), raw

    def forward(self, windows: torch.Tensor, sample_mask: torch.Tensor) -> torch.Tensor:
        normalized, _ = self.forward_with_raw(windows, sample_mask)
        return normalized


class ResNetLocalTemporalEncoder(nn.Module):
    """Apply the project's ResNet1D to each valid local motion window.

    The same per-window channel standardization as ``LocalTemporalEncoder`` is
    retained, and a validity channel distinguishes a partial tail window from
    real zeros.  Entirely padded tokens are excluded from BatchNorm statistics.
    """

    def __init__(self, in_channels: int, base_channels: int, embedding_dim: int) -> None:
        super().__init__()
        self.in_channels = _positive_int("in_channels", in_channels)
        self.embedding_dim = _positive_int("embedding_dim", embedding_dim)
    ...
    def _pooled_features(
        ...
        if not torch.is_floating_point(windows):
            raise TypeError("windows must have floating dtype.")
        sample_mask = sample_mask.bool()
        valid_tokens = sample_mask.any(dim=-1)
        if not torch.any(valid_tokens):
            raise ValueError("At least one valid local window is required.")
        weights = sample_mask.to(windows.dtype).unsqueeze(-1)
        counts = weights.sum(dim=2).clamp_min(1.0)
        mean = (windows * weights).sum(dim=2) / counts
        centred = (windows - mean.unsqueeze(2)) * weights
        variance = centred.square().sum(dim=2) / counts
        standard_deviation = torch.sqrt(variance + 1.0e-5)
        standardized = centred / standard_deviation.unsqueeze(2).clamp_min(1.0e-4)
        network_input = torch.cat(
            (standardized, sample_mask.to(windows.dtype).unsqueeze(-1)), dim=-1
        ).permute(0, 1, 3, 2)
        flat_input = network_input.reshape(
            -1, self.in_channels + 1, windows.shape[2]
        )
        flat_valid = valid_tokens.reshape(-1)
        encoded = windows.new_zeros((len(flat_valid), self.embedding_dim))
        encoded[flat_valid] = self.backbone(flat_input[flat_valid])
        return encoded.reshape(windows.shape[0], windows.shape[1], self.embedding_dim)
...
class SoftMotionCodebook(nn.Module):
    ...
    The primary protocol mirrors HAPPY's offline head construction at the
    architectural level: randomly initialized normalized prototypes and the
    ResNet1D encoder are optimized jointly from epoch one.  No KMeans result or
    pretrained HAPPY checkpoint is required.  This is equivalent to a
    weight-normalized bias-free linear prototype head because both the local
    embeddings and prototype rows are L2-normalized before their dot product.

    During primary training the deterministic straight-through branch uses
    exact argmax one-hot values in its forward pass and the soft posterior in
    its backward pass.  Gumbel sampling remains an explicit ablation only.
    This prevents both a continuous side channel and stochastic occupancy from
    hiding deterministic inference-time collapse.
    """
    ...
    def __init__(
        ...
        # Match HAPPY's randomly initialized DINOHead prototype directions.
        # Forward always normalizes rows, so their initial magnitude carries no
        # semantic information and cannot become a continuous shortcut.
    ...
    def normalize_learnable_prototypes_(self) -> None:
        ...
        HAPPY fixes the weight-normalization gain of its last-layer prototypes.
        The explicit codebook representation used here stores those directions
        directly, so this projection is the corresponding optimizer-side
        operation for the gradient-owned primary protocol.
        """
    ...
    def reset_ema_state(
        ...
        if cluster_sizes is None:
            ...
            if not torch.all(torch.isfinite(counts)) or torch.any(counts <= 0.0):
                raise ValueError("cluster_sizes must be finite and strictly positive.")
    ...
    def forward(
        ...
        *,
        ...
        if use_hard:
            ...
            # Exact deterministic one-hot values in the forward pass with the
            # posterior gradient in the backward pass.  Unlike Gumbel noise,
            # this exposes the same argmax occupancy during training and
            # inference, so a collapsed codebook cannot look healthy merely
            # because random samples happened to visit every code.
    ...
    def ema_update(
        ...
        *,
        ...
        # Each complete trial contributes total mass one regardless of length.
    ...
    def revive_unused_codes(
        ...
        Candidates are selected greedily by their distance from all currently
        active and already selected centres.  Rare but genuine primitives are
        preserved; only zero-count codes are replaced.
        """
        ...
        available = torch.ones(len(candidates), dtype=torch.bool, device=candidates.device)
...
class MotionTrajectoryModel(nn.Module):
    ...
    def __init__(
        ...
        **config_overrides: Any,
        ...
        cfg = self.config
        if cfg.local_encoder_type == "resnet1d":
            self.local_encoder = ResNetLocalTemporalEncoder(
    ...
    def forward(
        ...
        *,
        ...
        local_embeddings, local_embeddings_raw = self.local_encoder.forward_with_raw(
            windows, sample_mask
        )
        local_embeddings = local_embeddings.masked_fill(
            ~token_mask.unsqueeze(-1), 0.0
        )
        local_embeddings_raw = local_embeddings_raw.masked_fill(
            ~token_mask.unsqueeze(-1), 0.0
        )
        ...
        return {
            ...
            "local_embeddings_raw": local_embeddings_raw,
