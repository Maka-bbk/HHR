...
The classifier in this module has one deliberately narrow information path::
    ...
    raw trial -> local dynamics -> soft primitive ids --+
             -> raw kinematic state --------------------+-> trajectory GRU
             -> boundary / duration / transition -------+

In particular, the trajectory classifier never receives the continuous local
embedding, a quantized code vector, or a mean/max pool of either.  This keeps
the discrete motion-primitive trajectory falsifiable: classification can use
primitive identity, order, duration, transitions, boundaries, and a small
independent raw-state descriptor, but cannot silently fall back to the legacy
continuous pooled feature.
...
class SoftMotionCodebook(nn.Module):
    """K-way differentiable motion codebook with hard inference export."""
...
class MotionTrajectoryModel(nn.Module):
    ...
    def __init__(
        ...
        **config_overrides: Any,
        ...
        # Allowed classifier inputs: K primitive probabilities, raw-state
        # projection, boundary probability, transition probability, duration.
