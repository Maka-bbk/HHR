...
- 轨迹分类器只接收软动作元分配、顺序、边界/转移、持续时间/位置和物理状态；不接收内容特征的直接 mean/max pooling（均值/最大值池化）捷径。
...
完整试次 [B,C,T]
  ...
  -> 可微 K=32 VQ 动作元码本
  -> 软动作元分配 + 边界/转移 + 持续时间 + 状态序列
  -> masked trajectory encoder（掩码轨迹编码器）
  -> 轨迹嵌入 + old6 轨迹 logits（分类分数）
```

训练使用软码本分配以保持可微；hard token（硬标记）和按连续相同标记/预测边界形成的动作元区间仅用于评价、可视化和导出。原始边界锚点只覆盖完整原始帧；额外右侧 padding 尾帧对应的边界保持不确定，不进入边界监督。
...
每轮验证始终使用表中的完整基础权重和硬码本，不用当轮的 warmup 权重。J0-T 先按 validation trajectory macro-F1（验证轨迹宏平均 F1）选点，再以较低验证损失打破平局；J0-U 先按较低验证总损失选点，再以较高有效码字数打破平局。
...
顺序消融会打乱预测动作元 run block（连续动作元区块），保留区块内部帧顺序，并在 identity（恒等）与 shuffle（打乱）两侧都重算转移/持续时间、清零 learned-boundary（学习边界）入口。应主要看 `identity_control_minus_shuffled`；但状态序列也随区块一起打乱，所以首轮只能把下降归因于联合轨迹顺序，不能单独证明离散动作元顺序。若需分离贡献，应另做匹配训练的 token-only（仅标记）和 state-only（仅状态）消融。
...
| `complete.json` | 最后写入的完成标志，记录运行身份、`summary.json` 和选中检查点的 SHA256。 |
...
| `per_run_metrics.csv` | 每个成员的 All/Old/New/H/macro-F1、顺序 H-score 下降、有效码字数、token-subject NMI（标记—受试者归一化互信息）、选中轮次和目录。 |
| `aggregate_metrics.png` | J0-U/J0-T 的 All、Old、New、H-score 折均值及 bootstrap 误差条。 |

七折训练集高度重叠，只有七个外测受试者对；聚合 p 值和 bootstrap 区间只能作为筛查证据，不是强确认性结论。

## 暂停与 `--resume` 恢复

没有专门的暂停信号处理器。使用 `Ctrl+C` 或 PyCharm Stop（停止）时，当前未完成 epoch 可能丢失；`checkpoint_last.pt` 采用临时文件后原子替换，最近一个完整 epoch 可恢复。

恢复时对完全相同的输出目录和参数重新运行，并加 `--resume`。程序会校验运行身份，加载模型、优化器和调度器，从 `last_epoch+1` 继续，同时恢复最佳轮次和 patience。若成员已有 `complete.json`，会核对 profile/fold/seed/epochs 后直接返回已有 `summary.json`；CV runner（交叉验证运行器）会依次快速跳过已完成成员并恢复未完成成员。

当前检查点没有保存 Python、NumPy、Torch/CUDA 和 DataLoader（数据加载器）的随机数状态，因此恢复是“模型/优化器/epoch 级续跑”，不保证与从未中断的运行逐位相同。另因 `checkpoint_last.pt` 先写、`history.jsonl` 后追加，极窄的中断窗口可能使历史文件少一行，但恢复权威状态仍以检查点为准。
